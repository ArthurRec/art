...
from exchanger.builder import Builder
from exchanger.loader import ExchangeLoader
from exchanger.models import Exchange
from exchanger.serializers import ExchangeMessageSerializer


class ExchangeTestMixin(TestCase):
    def assert_basic_message_is_saved(self):
        self.assertEqual(3, Exchange.objects.count())
        self.assertTrue(
...
class ExchangeMessageSerializerTestCase(ExchangeTestMixin):
    ...
    def test_message_is_deserialized(self):
        ...
        self.assertDictEqual(self.serialized, serializer.validated_data)
...
class ExchangeLoaderTestCase(ExchangeTestMixin):
    ...
    def test_read(self, request):
        ...
        request.assert_called_with(
            method='GET',
            url="{0}{1}".format(self.loader.BASE_API_URL, today.isoformat())
        )
        self.assert_basic_message_is_saved()

    def test_read_period(self):
        with mock.patch.object(
                ExchangeLoader, 'read', return_value=None) as mock_read:
...
class ExchangeViewSetTestCase(APITestCase):
    def test_get_exchanges(self):
        today = datetime.date.today()
        url = "{0}?date={1}".format(
            reverse('exchanges-list'), today.isoformat())
